...
if (checkboxConfiguration.radioButtonGroup) {
    ...
    const radioButtons = checkboxConfiguration.radioButtonGroup.radioButtons;
    ...
    for (const radioButton of radioButtons) {
      ...
      radioLabel.setAttribute("for", `${checkBox.id}-${radioButton.value}`);
    ...
    checkBox.addEventListener("change", () => {
      const radioInputs = radioGroupDiv.querySelectorAll("input[type=radio]");
  
      for (const radioInput of radioInputs) {
        radioInput.disabled = !checkBox.checked;
  
        if (checkBox.checked && defaultCheckedButton !== null) {
          if (radioInput.value === checkboxConfiguration.radioButtonGroup.defaultCheckedButtonValue) {
            radioInput.checked = true;
          }
        }
      }
    ...
    if (defaultCheckedButton === null) {
      const defaultCheckedButtonValue = checkboxConfiguration.radioButtonGroup.defaultCheckedButtonValue;
      const radioInputs = radioGroupDiv.querySelectorAll("input[type=radio]");
